...
def web_search(query:str):
    ...
    resource_json_path = f'{current_path}/data/resource_{datetime.now().strftime("%Y%m%d%H%M%S")}.json'
    with open(
        resource_json_path,
        'w',
        encoding='utf-8'
...
def documents_to_chroma(documents, chunk_size=1000,chunk_overlap=100):
    ...
    stored_metadatas = vectorstore._collection.get(include=['metadatas'])
    stored_web_urls = [metadata['source'] for metadata in stored_metadatas['metadatas']]
...
def add_web_pages_json_to_chroma(json_file,chunk_size=1000,chunk_overlap=100):
    document = web_page_json_to_documents(json_file)
    documents_to_chroma(document,chunk_size,chunk_overlap)
...
if __name__ == "__main__":
    # result, resource_json_path = web_search.invoke("2026년 한국 영화 시장 전망")
    # print(result)
    # document = web_page_json_to_documents(f'{current_path}/data/resource_20260429174934.json')
    # splits = split_documents(document)
    
    # add_web_pages_json_to_chroma(f'{current_path}/data/resource_20260429174934.json')
    retrieved_docs = retrieve.invoke('왕과 사는 남자')
