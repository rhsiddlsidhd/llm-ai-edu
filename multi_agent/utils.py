...
def get_outline(current_path):
    ...
    if os.path.exists(f'{current_path}/data/ouline.md'):
        with open(f'{current_path}/data/ouline.md','r',encoding='utf-8') as f:
            outline = f.read()
...
def save_outline(current_path,outline):
    ...
    with open(f'{current_path}/data/ouline.md','w',encoding='utf-8') as f:
        f.write(outline)
    return outline        
