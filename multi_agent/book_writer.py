...
from utils import save_state, save_outline, get_outline
...
def business_analyst(state:State):
    ...
    business_analyst_system_prompt = PromptTemplate.from_template(
        """
        너는 책을 쓰는 AI 팀의 비즈니스 애널리스트로서, AI 팀의 진행상황과 "사용자 요구 사항"을 토대로, 현 시점에서 'ai_recommendation'과 최근 사용자의 발언을 바탕으로 요구 사항이 무엇인지 판단한다.
    ...
    messages.append(AIMessage(content=business_analyst_message))
...
def supervisor(state:State): # 슈퍼바이저
    print('\n\n ==============SUPERVISER==============')
...
def content_strategist(state:State):
    ...
    content_strategist_system_prompt = PromptTemplate.from_template(
        ...
        3. 책의 전반적인 세부 구조(chapter, section, sub-section)를 설계하고, sub_section하부의 주요 내용을 리스트 형태로 정리한다.
...
def communicator(state:State):
    ...
    if task_history[-1].agent != 'communicator':
        raise ValueError(f'Commnuicator가 아닌 agent가 대화를 시도하고 있습니다. \n {task_history[-1]}')
...
def vector_search_agent(state:State):
    ...
    현재 목차(outline)를 작성하는 데 필요한 정보를 확보하기 위해, 다음 내용을 활용해 적절한 벡터 검색을 수행하라
    ...
    for tool_call in search_plans.tool_calls:
        ...
        references['docs'].extend(retrieved_docs)
    
    unique_docs = []
    unique_page_content= set()
    
    for doc in references['docs']:
        if doc.page_content not in unique_page_content:
            unique_docs.append(doc)
            unique_page_content.add(doc.page_content)
    ...
    msg_str = f'[VECTOR_SEARCH_AGENT] 다음 질문에 대한 검색 완료: {queries}'
...
def web_search_agent(state:State):
    ...
    web_search_system_prompt = PromptTemplate.from_template(
        """
        너는 다른 AI Agent들이 수행한 작업을 바탕으로, 목차(outline) 작성에 필요한 정보를 웹 검색을 통해 찾아내는 Web Search Agent이다.
    ...
    llm_with_retriever = llm.bind_tools([web_search])
    
    web_search_chain = web_search_system_prompt | llm_with_retriever
    ...
    messages.append(AIMessage(content=msg_str))
...
def outline_reviewer(state:State):
    print('\n\n ============== OUTLINE REVIEWER ==============')
    outline_reviewer_system_template = PromptTemplate.from_template(
    """
    ...
    outline_reviewer_chain = outline_reviewer_system_template | llm 
...
state = State(
    messages=[
        SystemMessage(
            f"""
            너의 AI들은 사용자의 요구에 맞는 책을 쓰는 작가 팀이다.
            사용자가 사용하는 언어로 대화하라
            
            현재시간은 {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}이다.
